...
with cols[1]:
 ...
 st.markdown(f"""<div><h4 style="text-align:left; color: white; margin-top: -15px; ">Tavily - Webscraper</h4></div>""", unsafe_allow_html=True)
 st.markdown(f"""<div style="border-radius: 5px;">
  <h4 style="text-align:left; color: white;">
     <ul style="list-style-type: disc; margin-left: 1px; margin-top: -20px;">
...
if (fs_pdf_file is not None) and (covenants_pdf_file is not None):
 ...
 query = f"Check if company {df['Company Name'].iloc[0]} has breached any covenants based on the financial data provided, start your response with either 'Yes' or 'No'. Be detailed in your response."
 ...
 if query:
  st.write(agent_chain.run(
     "Is {df['Company Name'].iloc[0]} bankrupt currently? Start the response with either 'Yes' or 'No'.",
 ))
